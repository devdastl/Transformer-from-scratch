#This is version 3 of the code with single head attention. Following are new changes:
##usecase change:
#Now we have switched from bigram model to transformer model with single headed attention.
## Architecture change: 
#1. Added singleattentionhead class which will handle the single head attention.
#2. Updated forward pass to include single attention and then the output pass through linear layer.
#3. Added new parameter in model class to handle the head size.
...
class SimpleBigramModel(nn.Module):
    def __init__(self, embed_size, vocab_size, block_size, head_size):
        super().__init__()
        ...
        self.attention_head = SingleAttentionHead(self.block_size, embed_size, head_size)
        self.linear_layer_Head = torch.nn.Linear(self.head_size, self.vocab_size) #--> 32x8xvocab_size
    ...
    def forward(self, input, output=None):
        ...
        token_embeddings = token_embeddings + position_embedding #32x8x65 + 8x65 --> 32x8x65
        attn_output = self.attention_head(token_embeddings) #32x8x65 --> 32x8x16
        
        logits = self.linear_layer_Head(attn_output) #32x8x16 --> 32x8x65
...
def main():
    ...
    model_instance = SimpleBigramModel(vocab_size, vocab_size, block_size, vocab_size).to(device)
    if os.path.exists("bigram_model_v3.pth"):
        model_instance.load_state_dict(torch.load("bigram_model_v3.pth"))
    ...
    torch.save(model_instance.state_dict(), "bigram_model_v3.pth")
